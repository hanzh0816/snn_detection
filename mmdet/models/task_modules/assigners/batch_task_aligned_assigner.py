...
class BatchTaskAlignedAssigner(nn.Module):
    ...
    def forward(
        ...
        assigned_labels, assigned_bboxes, assigned_scores = self.get_targets(
            gt_labels, gt_bboxes, assigned_gt_idxs, fg_mask_pre_prior, batch_size, num_gt
        )
    ...
    def get_pos_mask(
        ...
        topk_metric = self.select_topk_candidates(
            alignment_metrics * is_in_gts, topk_mask=pad_bbox_flag.repeat([1, 1, self.topk]).bool()
        )
    ...
    def get_targets(
        ...
        batch_ind = torch.arange(end=batch_size, dtype=torch.int64, device=gt_labels.device)[
            ..., None
        ]
        ...
        assigned_scores = torch.where(
            force_gt_scores_mask > 0, assigned_scores, torch.full_like(assigned_scores, 0)
        )
